...
from ldm.models.diffusion.ddimella import DDIMSampler
from ldm.models.diffusion.plmsella import PLMSSampler
from inference import load_ckpt, prepare_batch, alpha_generator, set_alpha_scale
import inference

from ldm.ella import ELLA , AutoModelEmbedder

device = "cuda:5"
inference.device = device
AutoModelEmbedder1 = AutoModelEmbedder().to(device, dtype=torch.float16)
...
def run_batch(meta, config, starting_noise=None):
    ...
    for i in tqdm(range(len(meta))):
        ...
        context = AutoModelEmbedder1([meta[i]["prompt"]] * config.batch_size)

        
        if args.negative_prompt is not None:
            uc = AutoModelEmbedder1(config.batch_size * [args.negative_prompt])

        else:
            uc = AutoModelEmbedder1(config.batch_size * [""])
        ...
        #     timesteps=torch.tensor([i]).long().to(device)
        #     context=text_encoder(context0,timesteps).to(device)
        #     input['context']=context.to(device)
        #     input['timesteps']=timesteps.to(device)
        #     x=model(input)
        #     input['x']=x
        # samples_fake = autoencoder.decode(x)
        ...
        samples_fake = sampler.sample(S=steps, shape=shape, input=input, uc=uc, guidance_scale=config.guidance_scale,
                                      mask=inpainting_mask, x0=z0,ella=text_encoder)
        samples_fake = autoencoder.decode(samples_fake.half())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder", type=str, default="generation_llm", help="root folder for output")
    ...
    parser.add_argument("--seed", type=int, default=20010811)
    ...
    for r in res:  # [:2]
        meta_list_new.append(dict(
            ckpt="OUTPUT/Llama_test/tag00/checkpoint_00480001.pth",
            prompt=r["prompt"],
