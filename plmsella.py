...
class PLMSSampler(object):
    def __init__(self, diffusion, model, schedule="linear", alpha_generator_func=None, set_alpha_scale=None,ella=None):
        super().__init__()
    ...
    @torch.no_grad()
    def sample(self, S, shape, input, uc=None, guidance_scale=1, mask=None, x0=None,ella=None):
    ...
    def p_sample_plms(self, input, t, index, guidance_scale=1., uc=None, old_eps=None, t_next=None):
        ...
        def get_model_output(input):
            input["context"]= self.ella(self.context,input["timesteps"])
            e_t = self.model(input) 
            if uc is not None and guidance_scale != 1:
                unconditional_input = dict(x=input["x"], timesteps=input["timesteps"], context=self.ella(self.inputuc,input["timesteps"]), inpainting_extra_input=input["inpainting_extra_input"], grounding_extra_input=input['grounding_extra_input'])
        ...
        def get_x_prev_and_pred_x0(e_t, index):
            # select parameters corresponding to the currently considered timestep  TODO: is there any degraded performance?
            a_t = torch.full((b, 1, 1, 1), self.ddim_alphas[index], device=self.device, dtype=e_t.dtype)
            a_prev = torch.full((b, 1, 1, 1), self.ddim_alphas_prev[index], device=self.device, dtype=e_t.dtype)
            sigma_t = torch.full((b, 1, 1, 1), self.ddim_sigmas[index], device=self.device, dtype=e_t.dtype)
            sqrt_one_minus_at = torch.full((b, 1, 1, 1), self.ddim_sqrt_one_minus_alphas[index],device=self.device, dtype=e_t.dtype)
