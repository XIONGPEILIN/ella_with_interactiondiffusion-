...
class Trainer:
    def __init__(self, config, device="cuda"):
        ...
        del state_dict["text_encoder"]['transformer.text_model.embeddings.position_ids'] #add this line
        ...
        self.text_encoder  = ELLA(out_dim=768).to(self.device)
        self.AutoModelEmbedder = AutoModelEmbedder().to(self.device, dtype=torch.float16)
        ...
        if config.distributed:
            ...
            #self.llmencoder = DDP(self.llmencoder, device_ids=[config.local_rank], output_device=config.local_rank)
    ...
    def get_input(self, batch):
        ...
        context=self.AutoModelEmbedder(batch["caption"],)
        context = self.text_encoder(context,timesteps=t)
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            uc = self.AutoModelEmbedder(self.text_encoder.encode(batch_here * [""]))
            context = self.AutoModelEmbedder(self.text_encoder.encode(batch["caption"]))
